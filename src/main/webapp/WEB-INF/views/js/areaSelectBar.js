...
//문서가 준비되면 매개변수로 넣은 콜백함수를 실행해라의 의미
$(function() {
	//전국의 시도,구군의 명칭을 담은 배열 선언
	var arrSido = ["시/도 선택", "서울특별시", "인천광역시", "대전광역시", "광주광역시", "대구광역시", "울산광역시", "부산광역시", "경기도", "강원도", "충청북도", "충청남도","전라북도", "전라남도", "경상북도", "경상남도", "제주도"];
	var arrSidoDetail = {
		"시/도 선택" : ["구/군 선택"],
	...
	$("select[name=sido]").change(appendGugunOption);
	//sido 선택시 gugun select bar option 변경 함수
	function appendGugunOption(sido){
		sido = $(this).val();
		$("select[name=gugun] option").remove();
		for(var i=0;i<arrSidoDetail[sido].length;i++){
